...
export async function handleSetConfiguration (config, exitAfter = false) {
  logger.log(chalk.blue('Configuring AI Service:'));
  try {
    // --- 1. Ask for Service ---
    ...
    if (existingKey) {
      ...
      if (reuseKeyAnswer.reuseKey) {
        apiKey = existingKey;
        logger.log(chalk.green('Using existing API key.'));
      } else {
    ...
    logger.log(`  Service: ${chalk.bold(SUPPORTED_SERVICES[selectedService])}`);
    // Display the final model name
    logger.log(`  Model:   ${chalk.bold(finalSelectedModel)}`);
    ...
    if (error.isTtyError) {
      ...
      if (error.message.includes('canceled')) { // Or check specific error types if inquirer provides them
        ...
        // logger.error(error); // Optional: log the full error for debugging
      }
    }
