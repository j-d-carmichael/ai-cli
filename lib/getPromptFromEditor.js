...
export async function getPromptFromEditor (defaultValue = '') {
  ...
  try {
    ...
    logger.log(chalk.yellow(`Launching editor (${editor}) for prompt input...`));
    ...
    await new Promise((resolve, reject) => {
      ...
      editorProcess.on('close', (code) => {
        if (code === 0) {
          logger.log(chalk.green('Editor closed. Reading content...'));
          resolve();
        } else {
    ...
    try {
      ...
      logger.warn(chalk.yellow(`Warning: Failed to clean up temporary file/directory: ${cleanupError.message}`));
    }
  }
}
